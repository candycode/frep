...
std::string ReadFile( const char* fname )
{
	...
	if( !is.good() ) throw std::logic_error( "Cannot open file " + std::string( fname ) );
	std::string buffer;
	std::string output;
	while( is.good() )
	{
...
Primitives ReadPrimitives( const char* fname )
{
	...
	while( is.good() )
	{
		...
		if( !iss.good() ) throw std::logic_error( "Error reading primitive file" );
		std::string objname;
		std::string filename;
		std::string matFilename;
		iss >> objname;
		if( !iss.good() ) throw std::logic_error( "Error reading primitive file name" );
		iss >> matFilename;
		if( !iss.good() ) throw std::logic_error( "Error reading primitive maetrial file name" );
		iss >> filename;
		buffer.clear();
		std::string frepCode = ReadFile( filename.c_str() );
		if( frepCode.empty() ) throw std::logic_error( "Error reading from file " + filename );
		std::string matCode = ReadFile( matFilename.c_str() );
		if( matCode.empty() ) throw std::logic_error( "Error reading from file " + matFilename );
		...
		{
			frepCode.replace( frepCode.find( FREP_PLACEHOLDER ), FREP_PLACEHOLDER.size(), frepname );
		}
		while( frepCode.find( GRADIENT_PLACEHOLDER ) != std::string::npos )
		{
			frepCode.replace( frepCode.find( GRADIENT_PLACEHOLDER ), GRADIENT_PLACEHOLDER.size(), gradname );
		}
		while( frepCode.find( INTERSECT_PLACEHOLDER ) != std::string::npos )
		{
			frepCode.replace( frepCode.find( INTERSECT_PLACEHOLDER ), INTERSECT_PLACEHOLDER.size(), intersectname );
		}
		if( matCode.find( COLOR_PLACEHOLDER ) != std::string::npos )
		{
			matCode.replace( matCode.find( COLOR_PLACEHOLDER ), COLOR_PLACEHOLDER.size(), colorname );
		}
		primitives.push_back( PrimitiveEntry( objname, frepname, gradname, intersectname, colorname, invMatrixName,
			matrixName, invNormalMatrixName, normalMatrixName, frepCode, matCode ) );
	}
...
//------------------------------------------------------------------------------
std::string ReplaceNamesInComposite( const char* fname, const Primitives& primitives )
{
	std::string compositeCode = ReadFile( fname );
	if( compositeCode.empty() ) throw std::logic_error( "Cannot read composite code" );
	...
	while( s != std::string::npos )
	{
		...
		--id;
		if( id >= int( primitives.size() ) ) throw std::logic_error( "Error object index greater than number of objects" );
		const std::string tag = "$" + idx;
		compositeCode.replace( compositeCode.find( tag ), tag.size(), primitives[ id ].objname );
...
template < class T >
void L_( const std::string& m, const T& v ) { std::clog << m << '\t' << v << '\n'; }
...
try
{
	...
	if( !arguments.read( "-primitives", primitivesFileName ) ) throw std::logic_error( "Missing primitives file name" );
	if( !arguments.read( "-composite",  compositeFileName  ) ) throw std::logic_error( "Missing composite file name" );
	if( !arguments.read( "-compositeMat",  compositeMatFileName  ) ) throw std::logic_error( "Missing composite material file name" );
	if( !arguments.read( "-vert", vertexShaderFileName ) ) throw std::logic_error( "Missing vertex shader file name" );
	if( !arguments.read( "-frag", fragmentShaderFileName ) ) throw std::logic_error( "Missing fragment shader file name" );
	...
	const bool computeRayInFragmentShader = arguments.read( "-computeRayInFragmentShader" );
...
L_( "READ PRIMITIVES", primitives.size() );
	std::string compositeCode = ReplaceNamesInComposite( compositeFileName.c_str(), primitives );
L_( "REPLACED NAMES IN COMPOSITE", "ok" );
	std::string vertexShaderText = ReadFile( vertexShaderFileName.c_str() );
	std::string fragmentShaderText = ReadFile( fragmentShaderFileName.c_str() );
L_( "READ SHADERS", "ok" );
	std::string compositeMaterial = ReplaceNamesInComposite( compositeMatFileName.c_str(), primitives );
L_( "REPLACED NAMES IN MATERIAL", "ok" );
	...
	for( Primitives::iterator i = primitives.begin(); i != primitives.end(); ++i )
	{
		...
		//the manipTransoform of the primitive instance is filled with the transform modified
		//by the manipulator
		osg::ref_ptr< osg::Group > pg =  CreatePrimitiveSceneGraph( *i, vertexShaderText, fragmentShaderText );
L_( "\tPRIMITIVE", "ok" );
		primitiveNodes.push_back( pg );
		primitiveRoot->addChild( osg::get_pointer( pg ) );
	}
	osg::ref_ptr< osg::StateSet > primitiveSSet = primitiveRoot->getOrCreateStateSet();
	primitiveSSet->addUniform( new osg::Uniform( "deltaStep", dPrimitiveStep ) );
...
L_( "CREATED PRIMITIVE SCENEGRAPH", "ok" );
	...
	if( depthMap )
	{
		...
		extern osg::Camera* CreatePreRenderCamera( osg::Texture* depth,
                                    ...
                                    osg::Texture* normals );
		osg::ref_ptr< osg::TextureRectangle > t = GenerateDepthTextureRectangle();
		depthCamera = CreatePreRenderCamera( osg::get_pointer( t ), 0, 0 );
	    depthCameraViewportUniform = new osg::Uniform( "ssaoviewport", osg::Vec4() );
	    osg::ref_ptr< osg::StateSet > ss = depthCamera->getOrCreateStateSet();
		compositeRoot->getOrCreateStateSet()->setTextureAttributeAndModes( 1, osg::get_pointer( t ) );
		compositeRoot->getOrCreateStateSet()->addUniform( new osg::Uniform( "depthMap", 1 ) );
		compositeRoot->getOrCreateStateSet()->addUniform( osg::get_pointer( depthCameraViewportUniform ) );
	...
	if( optimizedComposite )
	{
		...
		compositeRoot->addChild( CreateOptimalCompositeSceneGraph( primitives,
														compositeCode,
														compositeMaterial,
														vertexShaderText,
														fragmentShaderText,
	...
	{
		compositeRoot->addChild( CreateCompositeSceneGraph( primitives,
														compositeCode,
														compositeMaterial,
														vertexShaderText,
														fragmentShaderText,
	...
	osg::ref_ptr< osg::StateSet > compositeSSet = compositeRoot->getOrCreateStateSet();
	compositeSSet->addUniform( new osg::Uniform( "deltaStep", dCompositeStep ) );
	if( !compositeTexture.empty() )
    {
		if( !cubeMap )
		{
			osg::ref_ptr< osg::Texture2D > displacementTexture = new osg::Texture2D;
			osg::ref_ptr< osg::Image > image = osgDB::readImageFile( compositeTexture );
			if( !image ) throw std::logic_error( "Cannot read image " + compositeTexture );
			const int textureUnit = 0;
			displacementTexture->setImage( osg::get_pointer( image ) );
			// IMPORTANT TO KEEP NEAREST IF NOT ARTIFACTS AT x=0 ARISE!!
			displacementTexture->setFilter(osg::Texture::MIN_FILTER, osg::Texture::NEAREST );//LINEAR_MIPMAP_LINEAR);
			displacementTexture->setFilter(osg::Texture::MAG_FILTER, osg::Texture::NEAREST );//LINEAR_MIPMAP_LINEAR);
			...
			displacementTexture->setWrap( osg::Texture::WRAP_S, osg::Texture::REPEAT );
			displacementTexture->setWrap( osg::Texture::WRAP_T, osg::Texture::REPEAT );
			compositeSSet->setTextureAttributeAndModes( textureUnit, osg::get_pointer( displacementTexture ) );
			compositeSSet->addUniform( new osg::Uniform( compositeTextureUniform.c_str(), textureUnit ) );
			//compositeSSet->setMode( GL_BLEND, osg::StateAttribute::ON );
		}
		else
		{
			osg::ref_ptr< osg::TextureCubeMap > cubemap = new osg::TextureCubeMap;
			const std::string::size_type FILETYPE_SEPARATOR = compositeTexture.rfind( "." );
			const std::string tname( compositeTexture, 0, FILETYPE_SEPARATOR );
			const std::string ext( compositeTexture, FILETYPE_SEPARATOR );
			std::clog << "\n****************** " << FILETYPE_SEPARATOR << ' ' << tname << ' ' << ext << " ***********************\n"; 
			osg::ref_ptr< osg::Image > imagePosX = osgDB::readImageFile( tname + "_R" + ext );
			osg::ref_ptr< osg::Image > imageNegX = osgDB::readImageFile( tname + "_L" + ext );
			osg::ref_ptr< osg::Image > imagePosY = osgDB::readImageFile( tname + "_T" + ext );
			osg::ref_ptr< osg::Image > imageNegY = osgDB::readImageFile( tname + "_B" + ext );
			osg::ref_ptr< osg::Image > imagePosZ = osgDB::readImageFile( tname + "_N" + ext );
			osg::ref_ptr< osg::Image > imageNegZ = osgDB::readImageFile( tname + "_F" + ext );
			...
			if (imagePosX && imageNegX && imagePosY && imageNegY && imagePosZ && imageNegZ )
			{
				...
				cubemap->setFilter(osg::Texture::MIN_FILTER, osg::Texture::LINEAR_MIPMAP_NEAREST);
				cubemap->setFilter(osg::Texture::MAG_FILTER, osg::Texture::LINEAR);
			}
			else throw std::logic_error( "Cannot read cubemap " + compositeTexture );
			const int textureUnit = 0;
			compositeSSet->setTextureAttributeAndModes( textureUnit, osg::get_pointer( cubemap ) );
			compositeSSet->addUniform( new osg::Uniform( compositeTextureUniform.c_str(), textureUnit ) );
		}
    }
...
L_( "CREATED COMPOSITE SCENEGRAPH", "ok" );
    ...
    // view one
    {
        ...
        view->getCamera()->setName("Primitives cam");
       	view->getCamera()->setViewport(new osg::Viewport(0,0, traits->width/2, traits->height ) );
        view->getCamera()->setGraphicsContext(gc.get());
        view->setCameraManipulator(new osgGA::TrackballManipulator);
		if( computeRayInFragmentShader )
		{
			osg::ref_ptr< osg::Uniform > vp = new osg::Uniform( "viewport", osg::Vec4() );
			primitiveRoot->getOrCreateStateSet()->addUniform( osg::get_pointer( vp ) );
			view->getCamera()->setPreDrawCallback( new CameraAspectRatioCBack( osg::get_pointer( vp ) ) );
		}
		else view->getCamera()->setPreDrawCallback( new CameraAspectRatioCBack );
    ...
    // view two
    {
		...
		if( depthCamera != 0 )
		{
			...
			g->getOrCreateStateSet()->addUniform( new osg::Uniform( "ssao", 1 ) );
			...
			depthCamera->setViewport(new osg::Viewport(0,0, traits->width/2, traits->height ) );
			depthCamera->getOrCreateStateSet()->addUniform( new osg::Uniform( "ssao", 0 ), osg::StateAttribute::OVERRIDE );
		}
		else view->setSceneData( osg::get_pointer( compositeRoot ) );
		view->getCamera()->setName("Composite camera");
        view->getCamera()->setViewport(new osg::Viewport(traits->width/2,0, traits->width/2, traits->height));
        view->getCamera()->setGraphicsContext(gc.get());
        view->setCameraManipulator(new osgGA::TrackballManipulator);
		view->addEventHandler( new osgViewer::WindowSizeHandler );
		compositeViewCamera = view->getCamera();
		// Needed !! ??; needs more investigation to understand why so
		compositeViewCamera->setComputeNearFarMode( osg::CullSettings::DO_NOT_COMPUTE_NEAR_FAR );
		if( computeRayInFragmentShader )
		{
			osg::ref_ptr< osg::Uniform > vp = new osg::Uniform( "viewport", osg::Vec4() );
			compositeRoot->getOrCreateStateSet()->addUniform( osg::get_pointer( vp ) );
			if( !compositeBoxes.empty() )
			{
				view->getCamera()->setPreDrawCallback( CreateOptimalCompositeCameraCallback(
													primitives,
													primitiveNodes,
													compositeBoxes,
			...
			{
				view->getCamera()->setPreDrawCallback( CreateCompositeCameraCallback(
													primitives,
	...
	//depthCamera->setPostDrawCallback( new SnapImage( "C:\\tmp\\depth.rgb" ) );
	...
	else
	{
        ...
        viewer.realize();
		SyncCameraNode sn( compositeViewCamera, osg::get_pointer( depthCamera ), osg::get_pointer( depthCameraViewportUniform ) );
        while( !viewer.done() ) 
        {
            viewer.advance();
